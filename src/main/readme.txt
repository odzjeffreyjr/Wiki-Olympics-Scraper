...
All you need to do is run the Querybase.java main function, and you will be
...
information finding logic. Note that question 7 is not entirely accurate and
may run slower than most other functions because we are relying on an API call
to the openStreetMap API that is not the fastest API.

The wildcard question is question 8 and might answer an interesting question on longevity.
...
future and while grading happens. If there are changes in Wikipedia pages, the following may be affected:
...
Wins:
1. Obtained skills in the utilisation of JSoup and Regex for web scraping.
2. I can now implement further my lifelong personal project based on the new skills gained in data collection.
3. I was able to debug many issues that arose such as wrong answers and incorrect table reading by
    doing dry runs on the Wikipedia page and the code.

Feel free to play the game and experiment with various inputs. This code is reliable.

Extra credit:
I implemented the second extra credit task and you can get the extra credit files by answering question 10 in the
Querybase. The algorithm will return to you the relevant folder where the files have been stored, which is expected
to be under a flags folder.
